...
def read_profile(path: Path) -> dict[str, object]:
    ...
    for field in ("name", "model", "model_reasoning_effort"):
...
def audit(cwd: Path, codex_home: Path, parent_model: str | None) -> dict[str, object]:
    ...
    for scope, path in profile_candidates(cwd, codex_home):
        ...
        valid = model == expected_model and effort == expected_effort
        found[name].append(
            {
                "scope": scope,
                "path": str(path),
        ...
        if not valid:
            violations.append(
                ...
                f"expected {expected_model}/{expected_effort}"
            )
